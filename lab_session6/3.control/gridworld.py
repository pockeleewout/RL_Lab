...
class Grid:
    ...
    def _getLegacyText(self):
        t = [[self.data[x][y] for x in range(self.width)] for y in
             range(self.height)]
        t.reverse()
        return t
...
def parseOptions():
    ...
    optParser.add_option('-w', '--windowSize', metavar="X", type='int',
                         dest='gridSize', default=150,
    ...
    optParser.add_option('-s', '--speed', action='store', metavar="S",
                         type=float,
    ...
    optParser.add_option('-v', '--valueSteps', action='store_true',
                         default=False,
...
if __name__ == '__main__':
    ...
    if not opts.textDisplay:
        ...
        display = graphicsGridworldDisplay.GraphicsGridworldDisplay(mdp,
                                                                    opts.gridSize,
                                                                    opts.speed)
    try:
        display.start()
    ...
    if opts.agent == 'value':
        ...
        a = policyIterationAgents.PolicyIterationAgent(mdp, opts.discount,
                                                       opts.iters)
    elif opts.agent == 'q':
        ...
        if not opts.manual and opts.agent in ['value', 'policy']:
            if opts.valueSteps:
                for i in range(opts.iters):
                    tempAgent = valueIterationAgents.ValueIterationAgent(mdp,
                                                                         opts.discount,
                                                                         i)
                    display.displayValues(tempAgent,
                                          message="VALUES AFTER " + str(
                                              i) + " ITERATIONS")
                    display.pause()

            display.displayValues(a, message="VALUES AFTER " + str(
                opts.iters) + " ITERATIONS")
            display.pause()
            display.displayQValues(a, message="Q-VALUES AFTER " + str(
                opts.iters) + " ITERATIONS")
            display.pause()
    except KeyboardInterrupt:
        sys.exit(0)
    ...
    if not opts.quiet:
        if opts.manual and opts.agent == None:
            ...
            if opts.agent == 'random': displayCallback = lambda \
                state: display.displayValues(a, state, "CURRENT VALUES")
            if opts.agent == 'value': displayCallback = lambda \
                state: display.displayValues(a, state, "CURRENT VALUES")
            if opts.agent == 'q': displayCallback = lambda \
                state: display.displayQValues(a, state, "CURRENT Q-VALUES")
    ...
    if opts.manual:
        decisionCallback = lambda state: getUserAction(state,
    ...
    if opts.episodes > 0:
        print()
        print(("AVERAGE RETURNS FROM START STATE: " + str(
            (returns + 0.0) / opts.episodes)))
        print()
        print()
